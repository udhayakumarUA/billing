angular.module('posApp')
  .controller('returnlistCtrl', function($scope,$http,verifyDelete,$mdToast) {
    $scope.rows=20;
    $scope.count=0;
    $scope.currentpage=1;
    $scope.return=[];
    ...
    $http.post('/return/getreturncount').then(function (res) {
    ...
    $scope.more=function () {
      if($scope.searchtext=='') {
        ...
        $http.post('/return/getall', {skip: $scope.skip, limit: $scope.rows,category:$scope.statustype}).then(function (res) {
          console.log(res)
        ...
        $scope.skip = (($scope.currentpage - 1) * parseInt($scope.rows));
        $http.post('/return/searchreturn', {searchtext:$scope.searchtext,skip: $scope.skip, limit: $scope.rows,category:$scope.statustype}).then(function (res) {
          $scope.return = res.data.returns;
        })
      }
    };
    ...
    $scope.search=function (searchtext) {
      $scope.currentpage=1;
      $scope.skip=(($scope.currentpage-1)*parseInt($scope.rows));
      $http.post('/return/searchreturn',{searchtext:searchtext,skip:$scope.skip,limit:$scope.rows,category:$scope.statustype}).then(function (res) {
        $scope.count=res.data.count;
        $scope.totalpages=Math.ceil($scope.count/parseInt($scope.rows));
        $scope.return=res.data.returns;
      })
