angular.module('posApp')
  .controller('returneditCtrl', function($scope,$http,$mdDialog,$stateParams,verifyDelete,$mdToast,$state,$filter,$timeout,hotkeys) {
    $scope.customercount=0;
    $http.post('/return/singlereturn',{pid:$stateParams.pid}).then(function (res) {
      console.log(res);
      ...
      $scope.reason=$scope.return.reason;
      $scope.notes=$scope.return.notes;
      $scope.billdate=new Date($scope.return.returndate);
      $scope.returnid = res.data.returns.refid
        $scope.orginalid = res.data.returns.refid
        $scope.checkid(returnid);
    })
    $scope.checkid = (id) => {
      if (id == undefined) {
  ...
  $mdDialog.hide();
    ...
    $scope.updatebill=function () {
      ...
      $http.post('/return/updatebill',{
        ...
        $state.go('singlereturn',{pid:$stateParams.pid})
