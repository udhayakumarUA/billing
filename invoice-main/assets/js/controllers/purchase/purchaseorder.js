angular.module('posApp')
    ...
    //getting purchase order ID
     if($stateParams.key){
      $http.post('/purchaseorder/purchasebyid',{id:$stateParams.id}).then(function(res){
          ...
          $scope.orderid = res.data.result.porid
          $scope.orginalid = res.data.result.porid
          $scope.checkid(orderid);
     })
  ...
  $scope.checkid = (id) => {
    if (id == undefined) {
      ...
      return $http.post('/purchaseorder/checkid', { id: id }).then(function (res) {
        console.log(res)
      }).catch((err) => {
        console.log(err)
    ...
    $scope.updatepurchase=function () {
      ...
      $http.post('/purchaseorder/updatepurchase',{
       ...
       $location.path(`/purchaseordersingle/${$stateParams.id}`)
      })
      .catch(error=> {
        $scope.err = error.data.err
        $scope.success(error.data.err)
