angular.module('posApp')
  .controller('salesorderCtrl', function ($scope, $mdDialog, $http, $stateParams, $location, $timeout, $state, $mdToast,$filter,hotkeys) {
    ...
    //getting estimate ID
      ...
      $http.post('/salesorder/Salesorderbyid', { id: $stateParams.id }).then(function (res) {
        ...
        $scope.salesorderid = res.data.result.soid
        $scope.orginalid =  res.data.result.soid
      ...
      $http.post('/salesorder/getid').then(function (res) {
        ...
        $scope.salesorderid = res.data.id
        $scope.oldnumber = res.data.id
        $scope.totalpages = Math.ceil($scope.count / parseInt($scope.rows));
      })
    }
    //Check wether the Sales Order ID Already exist
    $scope.checkid = (id) => {
      if (id == undefined) {
        $scope.err = 'The ID field should not be empty'
        return
      }
      else if ($stateParams.key && $scope.orginalid==id){
        return
      }
      else if(id!=$scope.oldnumber)
        {

        return $http.post('/salesorder/checkid', { id: id }).then(function (res) {
          console.log(res)
        }).catch((err) => {
          console.log(err)
    ...
    $scope.addsalesorder = function () {
      ...
      if($scope.selectedsp){
        ...
        var salespersonname = $scope.selectedsp.name
      }else{
        var salespersonid = '';
        var salespersonname = '';
      }

      $http.post('/salesorder/addSalesorder', {
        ...
        console.log(res)
        $scope.success('Sale Order Added Successfully');
        $state.go('salesordersingle',{id:res.data.result.id});
      })
    };
    $scope.updatesalesorder = function () {
      ...
      $http.post('/salesorder/updateSalesorder', {
        id: $stateParams.id,
        soid:$scope.salesorderid,
        oldid:$scope.orginalid,
        user: $scope.selectedcustomer.id,
        placeofsuply: $scope.selectedcustomer.plos,
        status: $scope.status,
        startdate: startdate,
        ...
        console.log(res)
        $scope.success('Sales Order updated');
        $location.path('/salesordersingle/' + $stateParams.id)
      }).catch(err=>{
  ...
  $scope.cancel = function() {
    ...
    $scope.calculatetotal = function () {
      ...
      $scope.cgstprice=$scope.cgstprice.toFixed(2);
      $scope.sgstprice=$scope.sgstprice.toFixed(2);
      $scope.igstprice=$scope.igstprice.toFixed(2);
      $scope.cessprice=$scope.cessprice.toFixed(2);
