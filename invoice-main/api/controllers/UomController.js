...
module.exports = {
  adduom:(req,res)=>{
    Uom.create({
      ...
      if(err) return res.negotiate(err);
        Uom.find({type:'C',sort:'createdAt DESC'}).exec((err,uoms)=> {
          if (err) return res.negotiate(err)
  ...
  getuom:(req,res)=>{
    Uom.find({type:'C',limit:10}).exec((err,uoms)=>{
      if(err) return res.negotiate(err)
      Uom.count({type:'C'}).exec((err,count)=>{
        if(err) return res.negotiate(err)
  ...
  getmoreuom:(req,res)=>{
    Uom.find({type:'C',skip:req.param('skip'),limit:10}).exec((err,uoms)=>{
      if(err) return res.negotiate(err)
      Uom.count({type:'C'}).exec((err,count)=>{
        if(err) return res.negotiate(err)
