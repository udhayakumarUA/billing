...
module.exports = {
    //generating the Return id
    getsid: (req, res) => {

      Return.find({ sort: 'refid DESC' })
      .populate('user')
          .exec((err, result) => {
    ...
    checkid:(req,res)=>{
      Return.findOne({ refid: req.param('id')})
      .exec((err, ReturnId) => {
          if (err) { return res.negotiate(err) }
          if(ReturnId)
          return res.json(401, { err: 'Return ID Already Exist' });
          else
  ...
  addbill:(req,res)=>{
    if(!req.param('returnid')){
      return res.json(401, { err: 'The Return Number field should not be empty' });              
    }
    let retnumber;
    Return.findOne({refid:req.param('returnid'),limit:1}).exec((err,est)=>{
      if (err) return res.negotiate(err);
      if(est){
        return res.json(401, { err: 'Return ID Already Exist' });        
      }
      ...
      retnumber= 'RET-' + value.padStart(7, 0);
      }
        ...
        Returnproduct.create(result).exec((err,returnproductcreated)=>{
          ...
          Returnproduct.find({return:returncreated.id,groupBy:['productid'],sum:['quantity']})
            .exec((err,grouped)=>{
              ...
              return res.json({message:'created',refid:retnumber})
            })
        })
      })
    })
