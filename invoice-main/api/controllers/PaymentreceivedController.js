...
module.exports = {
  ...
  addbulkpayment: (req, res) => {
    ...
    console.log(amount)
    var bills = [];
    Pos.find({user: req.param('user'),status:{"!":"Paid"}, sort: 'createdAt ASC' }).exec((err, pos) => {
      if (err) return res.negotiate(err);
      if(pos.length>=1){
        async.each(pos,(payment,cb)=>{
          //this function update the reduced balance and status to the database.
          assigndata=(balance,status)=>{
            Pos.update({id:payment.id},{balance:balance,status:status})
            .exec((err,finished)=>{
              if (err) cb(error)
              cb()
            })
  ...
  reportbypayment:(req,res)=>{
    Paymentreceived.find({ 
      where:{paymentmode:{'contains':req.param('mode')},"paymentdate" : { ">=" :req.param('startdate'), "<=" : req.param('enddate')}},
      sum:['amount']})
    .exec((err,data)=>{
        if (err) {return res.negotiate(err)}
        Paymentreceived.find({ where:{paymentmode:{'contains':req.param('mode')},"paymentdate" : { ">=" :req.param('startdate'), "<=" : req.param('enddate')}}})
        .exec((err,result)=>{
        if (err) {return res.negotiate(err)}
