...
module.exports = {
  ...
  checkid:(req,res)=>{
    Purchase.findOne({ refid: req.param('id')})
    .exec((err, PurchaseId) => {
        if (err) { return res.negotiate(err) }
        if(PurchaseId)
        return res.json(401, { err: 'Purchase ID Already Exist' });
        else
  ...
  addbill:(req,res)=>{
    if(!req.param('purchaseid')){
      return res.json(401, { err: 'The Purchase Number field should not be empty' });              
    }
    let purnumber;
    Purchase.findOne({refid:req.param('purchaseid'),limit:1}).exec((err,est)=>{
      if (err) return res.negotiate(err);
      if(est){
        return res.json(401, { err: 'Purchase ID Already Exist' });        
      }
      else{
      let autoinc = req.param('purchaseid');
      let value = autoinc.toString()
      purnumber= 'REF-' + value.padStart(7, 0);
      }
      Purchase.create({
        user:req.param('userid'),
        username:req.param('username'),
        purchaseid:purnumber,
        refid:req.param('purchaseid'),
        ...
        Purchaseproduct.create(result).exec((err,purchaseproductcreated)=>{
          ...
          Purchaseproduct.find({purchase:purchasecreated.id,groupBy:['productid'],sum:['quantity']})
            .exec((err,grouped)=>{
              ...
              return res.json({message:'created',refid:purnumber})
            })
        })
      })
    })
  ...
  getpaymentammount:(req,res)=>{
    Purchase.find({status:{"!":"Paid"},user:req.param('userid')}).sum('balance')
      .exec((err,amount)=>{
        if (err) {return res.negotiate(err)}
  ...
  singlepurchase:(req,res)=>{
    ...
    Purchase.findOne({purchaseid:req.param('pid')})
      ...
      .exec((err,purchase)=> {
        ...
        Contact.findOne({id:purchase.user.id})
        .populate('purchase',{or:[{status:'Partially Paid'},{status:'Draft'}]})
        .exec((err,singlecontact)=>{
          if(err){return res.negotiate(err)}
            return res.json({purchase: purchase,template:tem,singlecontact:singlecontact})
        })
      })
      })
  },
  ...
  updatebill:(req,res)=>{
    ...
    Purchase.findOne({refid:req.param('purchaseid'),limit:1}).exec((err,est)=>{
      if (err) return res.negotiate(err);
      if(est){
        purnumber = est.purchaseid
        if(est.refid!=req.param('orginalid')){
          return res.json(401, { err: 'Purchase ID Already Exist' });        
        }
      }
      else{
