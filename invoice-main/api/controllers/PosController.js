...
module.exports = {
  getinvoiceid:(req,res)=>{
    Pos.find({ sort: 'autoinc DESC' })
        .populate('user')
            .exec((err, result) => {
  ...
  checkid:(req,res)=>{
    Pos.findOne({ autoinc: req.param('id')})
    .exec((err, invoiceId) => {
        if (err) { return res.negotiate(err) }
        if(invoiceId)
        return res.json(401, { err: 'Invoice ID Already Exist' });
        else
  ...
  profitlossreport:(req,res)=>{
    Pos.find({where: { "date": { ">=": req.param('startdate'), "<=": req.param('enddate') }}})
    // .sum('maintotal','discount','shipping','adjustment')
    .exec((err,postotal)=>{
      if (err) {return res.negotiate(err)}
      let profits ={maintotal:0,discount:0,shipping:0,adjustment:0}
      postotal.filter(fil=>{
        profits.maintotal+= fil.maintotal==null?0:fil.maintotal
        profits.discount+= (fil.discount=="" ||fil.discount==null)?0:parseFloat(fil.discount)
        profits.shipping+= fil.shipping==null?0:fil.shipping
        profits.adjustment+= fil.adjustment==null?0:fil.adjustment
        // profits.total+= fil.maintotal+fil.discount+fil.shipping+fil.adjustment
      })
      Purchase.find({where: { "purchasedate": { ">=": req.param('startdate'), "<=": req.param('enddate') }}})
      .sum('maintotal')
      .exec((err,purchase)=>{
        Expenses.find({where: { "date": { ">=": req.param('startdate'), "<=": req.param('enddate') } } ,groupBy: ['expensetype'], sum: ['tax']})
          .exec((err, expense) => {
            let expensetotal=0
            expense.filter(fil=>{
              expensetotal+=fil.tax
            })
        return res.json({income:postotal,sale:profits,purchase:purchase,expense:expense,expensetotal:expensetotal})
      })
    })
    })
  },
...
//sale Balance report  sigle customers
getreportcbalance:(req,res)=>{
  console.log(req.param('id'))
  Pos.find({ where:{status:{"!":"Paid"},"user":req.param('id'),"date" : { ">=" :req.param('startdate'), "<=" : req.param('enddate')}},sum:['maintotal','totalsub','balance','totalcgst','totalsgst','totaligst','totalcess'],count:1})
    .exec((err,data)=>{
    if (err) {return res.negotiate(err)}
    Pos.find({where:{status:{"!":"Paid"},"user":req.param('id'),"date" : { ">=" :req.param('startdate'), "<=" : req.param('enddate')}}})
    .populate('user')
    .exec((err,result)=>{
    if (err) {return res.negotiate(err)}
    return res.json({amount:data,result:result})
...
getreportinvoices:(req,res)=>{
  console.log(req.param('id'))
  Pos.find({ where:{"salesperson":req.param('id'),"date" : { ">=" :req.param('startdate'), "<=" : req.param('enddate')}},sum:['maintotal','totalsub','balance','totalcgst','totalsgst','totaligst','totalcess'],count:1})
    .exec((err,data)=>{
    if (err) {return res.negotiate(err)}
    Pos.find({where:{"salesperson":req.param('id'),"date" : { ">=" :req.param('startdate'), "<=" : req.param('enddate')}}})
    .populate('user')
    .exec((err,result)=>{
    if (err) {return res.negotiate(err)}
    return res.json({amount:data,result:result})
  ...
  getpaymentammount:(req,res)=>{
    Pos.find({status:{"!":"Paid"},user:req.param('customerId')}).sum('balance')
      .exec((err,amount)=>{
        if (err) {return res.negotiate(err)}
...
getreportvendorbalance:(req,res)=>{
  console.log(req.param('id'))
  console.log(req.param('startdate'))
  console.log(req.param('enddate'))
  Purchase.find({ where:{status:{"!":"Paid"},"user":req.param('id'),"purchasedate" : { ">=" :req.param('startdate'), "<=" : req.param('enddate')}},sum:['maintotal','totalsub','balance','totalcgst','totalsgst','totaligst','totalcess'],count:1})
    .exec((err,data)=>{
    console.log(data)
  if (err) {return res.negotiate(err)}
  Purchase.find({where:{status:{"!":"Paid"},"user":req.param('id'),"purchasedate" : { ">=" :req.param('startdate'), "<=" : req.param('enddate')}}})
    .populate('user')
    .exec((err,result)=>{
    console.log(result)
  if (err) {return res.negotiate(err)}
  ...
  reportbyitem: (req, res) => {
    ...
    detailedgst: (req, res) => {
      ...
      .exec((err,data)=>{
        ...
        Pos.find({where:{"date" : { ">=" :req.param('startdate'), "<=" : req.param('enddate')}}})
        .populate('products',{groupBy:['cgst'],sum:['singlecgst']})
          .exec((err,cgstdata)=>{
            if (err) {return res.negotiate(err)}
            cgstp.filter(fil=>{
              cgstdata.filter(cgst=>{
                if(cgst.id ==fil.id){
                  fil.cgstp = cgst.products
                }
              })
              }) 
            Pos.find({where:{"date" : { ">=" :req.param('startdate'), "<=" : req.param('enddate')}}})
            .populate('products',{groupBy:['sgst'],sum:['singlecgst','singlesgst','singleigst','singlecess']})
              .exec((err,sgstp)=>{
                if (err) {return res.negotiate(err)}
                let ind = 0;
                cgstp.filter(fil=>{
                  sgstp.filter(sgst=>{
                    if(sgst.id ==fil.id){
                      fil.sgstp = sgst.products
                    }
                  })
                  }) 
                Pos.find({where:{"date" : { ">=" :req.param('startdate'), "<=" : req.param('enddate')}}})
                .populate('products',{groupBy:['igst'],sum:['singlecgst','singlesgst','singleigst','singlecess']})
                  .exec((err,igstp)=>{
                    if (err) {return res.negotiate(err)}
                    cgstp.filter(fil=>{
                        igstp.filter(igst=>{
                          if(igst.id ==fil.id){
                            fil.igstp = igst.products
                          }
                        })
                      })
                    Pos.find({where:{"date" : { ">=" :req.param('startdate'), "<=" : req.param('enddate')}}})
                    .populate('products',{groupBy:['cess'],sum:['singlecgst','singlesgst','singleigst','singlecess']})
                      .exec((err,cessp)=>{
                        if (err) {return res.negotiate(err)}
                        cgstp.filter(fil=>{
                            cessp.filter(cess=>{
                              if(cess.id ==fil.id){
                                fil.cess = cess.products
                              }
                            })
                          })
                          return res.json({result:cgstp})
                      })
                  })
              })
              })
...
//save invoice
  saveinvoice:(req,res)=>{
    ...
    Pos.findOne({autoinc:req.param('invoicenumber'),limit:1}).exec((err,posid)=>{
      if (err) return res.negotiate(err);
      if(posid){
        return res.json(401, { err: 'Invoice ID Already Exist' });        
      }
      ...
      Pos.create({
        user:req.param('userid'),
        username:req.param('username'),
        posid:'#'+req.param('invoicenumber'),
        autoinc:parseInt(req.param('invoicenumber')),
        type:'invoice',
        ...
        Posproducts.create(result).exec((err, posproductcreated) => {
          ...
          Posproducts.find({pos: poscreated.id, groupBy: ['productid'], sum: ['quantity']})
            .exec((err, grouped) => {
              ...
              //The below condition is used to check whether the  sales order data is converted into invoice.
              //If it is true change the status of the sales order to Invoiced
              if(req.param('statuschange')){
                if(req.param('salesorderid')){
                Salesorder.update({id:req.param('salesorderid')},{status:'Invoiced'})
                .exec((err,saleorder)=>{
                })
              }
              else{
                Estimate.update({id:req.param('estimateid')},{status:'Approved'})
                .exec((err,estimate)=>{
                })
              }
            }
...
//update invoice
    ...
    Pos.findOne({autoinc:req.param('invoicenumber'),limit:1}).exec((err,posid)=>{
      if (err) return res.negotiate(err);
      if(posid){
        if (posid.autoinc!=req.param('orginalid')) {return res.json(401, { err: 'Invoice ID Already Exist' })};        
      }
...
//all data
  ...
  singlepos:(req,res)=>{
    ...
    Pos.findOne({posid:req.param('posid')})
      ...
      if (err) return res.negotiate(err);
        ...
        Contact.findOne({id:pos.user.id})
        .populate('pos',{or:[{status:'Partially Paid'},{status:'Draft'}]})
        .exec((err,singlecontact)=>{
          if (err) return res.negotiate(err);
        User.findOne({key:req.param('key')}).exec((err,mainuser)=>{
          if (err) return res.negotiate(err);
          return res.json({pos:pos,mainuser:mainuser,template:tem,result:results,singlecontact:singlecontact})
    })
      })
    })
    ...
    Defaulttem.findOne({template:'pos'}).exec((err,possize)=> {
      ...
      Defaulttem.findOne({template: 'purchase'}).exec((err, purchasesize) => {
        ...
        Defaulttem.findOne({template: 'estimate'}).exec((err, estimatesize) => {
          if (err) return res.negotiate(err);
            Defaulttem.findOne({template: 'salesorder'}).exec((err, salesordersize) => {
              if (err) return res.negotiate(err);
                return res.json({possize:possize,purchasesize:purchasesize,estimatesize:estimatesize,salesordersize:salesordersize})
          })
        })
      })
    })
  ...
  getoverduepayment:(req,res)=>{
    $today = new Date();
    $yesterday = new Date($today);
    $yesterday.setDate($today.getDate() - 1);
      Pos.find({ status:['Partially Paid','Draft'],
      duedate: { "<=": $yesterday} ,
      sort: 'duedate ASC',
      limit: req.param('limit'),
      skip: req.param('skip')
      ...
      .exec((err, overdue) => {
        if (err) return res.negotiate(err);
        Pos.count({
          status:['Partially Paid','Draft'],
          duedate: { "<=": $yesterday} ,
      }).exec((err, count) => {
          if (err) return res.negotiate(err);
          return res.json({overdue: overdue,count:count})
        })
      })
  },
  searchOverDuePayment: (req, res) => {
    $today = new Date();
    $yesterday = new Date($today);
    $yesterday.setDate($today.getDate() - 1);
    Pos.find({
...
getsaledetails:(req,res)=>{
  ...
  if(req.param('type')=='payables'){
    name=Purchase;
  }
  else if(req.param('type')=='todaysale'){
    params.status.push('Paid')
    params = Object.assign(params,{date:req.param('date')})
    countdata ={where:{"date": { ">=": req.param('date'), "<=": req.param('date') } }}
  }
    name.find(params)
    .populate('user')
    .exec((err, result) => {
      if (err) return res.negotiate(err);
      name.count(countdata).exec((err, count) => {
        if (err) return res.negotiate(err);
...
searchsaledetails: (req, res) => {
  ...
  if(req.param('type')=='receivables'){
    params = Object.assign(params,{ or: [{posid: {'contains': req.param('searchtext')}},
    {username: {'contains': req.param('searchtext')}}]})
    countdata = {status:status,or: [{posid: {'contains': req.param('searchtext')}},
    {username: {'contains': req.param('searchtext')}}]}
  }
  if(req.param('type')=='payables'){
    name=Purchase;
    params = Object.assign(params,{ or: [{purchaseid: {'contains': req.param('searchtext')}},
    {username: {'contains': req.param('searchtext')}}]})
    countdata = {status:status,or: [{purchaseid: {'contains': req.param('searchtext')}},
    {username: {'contains': req.param('searchtext')}}]}
  }
  else if(req.param('type')=='todaysale'){
    params.status.push('Paid')
    params = Object.assign(params,{date:req.param('date')},{ or: [{posid: {'contains': req.param('searchtext')}},
    {username: {'contains': req.param('searchtext')}}],})
    countdata ={where:{or: [{posid: {'contains': req.param('searchtext')}},
    {username: {'contains': req.param('searchtext')}}], "date": { ">=": req.param('date'), "<=": req.param('date') } }}
  }
