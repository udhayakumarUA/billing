...
module.exports = {
  paymentmade:(req,res)=>{
    Purchase.findOne({userid:req.param('userid'),purchaseid:req.param('billid')}).exec((err,purchase)=>{
      if (err) return res.negotiate(err);
      if(purchase.balance<req.param('amount')){
        return res.json(401,{err:'payment failed'})
      }

      Paymentmade.find({userid:req.param('userid'),sort:'count DESC',limit:1}).exec((err,payid)=>{
        if (err) return res.negotiate(err);
        if(payid.length>=1) {
  ...
  reportbypaymentmade:(req,res)=>{
    Paymentmade.find({ where:{userid:req.param('userid'),"paymentdate" : { ">=" :req.param('startdate'), "<=" : req.param('enddate')}},sum:['amount']})
    .exec((err,data)=>{
        if (err) {return res.negotiate(err)}
        Paymentmade.find({where:{ userid:req.param('userid'),"paymentdate" : { ">=" :req.param('startdate'), "<=" : req.param('enddate')}}})
        .exec((err,result)=>{
        if (err) {return res.negotiate(err)}
  ...
  getall:(req,res)=>{
    Paymentmade.find({userid:req.param('userid'),sort:'count DESC',limit:req.param('limit'),skip:req.param('skip')})
      .populate('user')
      .exec((err,payments)=> {
        if (err) return res.negotiate(err);
        Paymentmade.count({userid:req.param('userid')}).exec((err, count) => {
          if (err) return res.negotiate(err);
  ...
  searchpayment:(req,res)=>{
    Paymentmade.find({userid:req.param('userid'),
      ...
      .exec((err,payments)=> {
        if (err) return res.negotiate(err);
        Paymentmade.count({userid:req.param('userid'),
          or:[{username:{'contains':req.param('searchtext')}}
            ,{paymentid:{'contains':req.param('searchtext')}}
            ,{billid:{'contains':req.param('searchtext')}}]
        })
  ...
  singlepaymade:(req,res)=>{
    ...
    Paymentmade.findOne({userid:req.param('userid'),paymentid:req.param('id')})
      .populate('user')
      .exec((err,payment)=>{
        if (err) return res.negotiate(err);
