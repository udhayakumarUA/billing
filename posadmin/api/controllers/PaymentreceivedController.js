...
module.exports = {
  paymentreceived:(req,res)=>{
    Pos.findOne({userid:req.param('clientid'),posid:req.param('billid')}).exec((err,pos)=>{
      if (err) return res.negotiate(err);
      if(pos.balance<req.param('amount')){
        return res.json(401,{err:'payment failed'})
      }

    Paymentreceived.find({userid:req.param('clientid'),sort:'count DESC',limit:1}).exec((err,payid)=>{
      if (err) return res.negotiate(err);
      if(payid.length>=1) {
  ...
  reportbypayment:(req,res)=>{
    Paymentreceived.find({ userid:req.param('clientid'),where:{"paymentdate" : { ">=" :req.param('startdate'), "<=" : req.param('enddate')}},sum:['amount']})
    .exec((err,data)=>{
        if (err) {return res.negotiate(err)}
        Paymentreceived.find({ userid:req.param('clientid'),where:{"paymentdate" : { ">=" :req.param('startdate'), "<=" : req.param('enddate')}}})
        .exec((err,result)=>{
        if (err) {return res.negotiate(err)}
  ...
  getall:(req,res)=>{
    Paymentreceived.find({userid:req.param('clientid'),sort:'count DESC',limit:req.param('limit'),skip:req.param('skip')})
      .populate('user')
      .exec((err,payments)=> {
      if (err) return res.negotiate(err);
      Paymentreceived.count({userid:req.param('clientid')}).exec((err, count) => {
        if (err) return res.negotiate(err);
  ...
  searchpayment:(req,res)=>{
    Paymentreceived.find({userid:req.param('clientid'),
      ...
      .exec((err,payments)=> {
      if (err) return res.negotiate(err);
      Paymentreceived.count({userid:req.param('clientid'),
        or:[{username:{'contains':req.param('searchtext')}}
          ,{paymentid:{'contains':req.param('searchtext')}}
          ,{billid:{'contains':req.param('searchtext')}}]
      })
  ...
  singlepayment:(req,res)=>{
    Paymentreceived.findOne({userid:req.param('clientid'),paymentid:req.param('id')})
      .populate('user')
      .exec((err,payment)=>{
        if (err) return res.negotiate(err);
