angular.module('posApp')
  .controller('SinglereturnCtrl', function($scope,$http,$stateParams,verifyDelete,$mdToast,$state) {
    $scope.disable=false;
    $http.post('/return/singlereturn',{pid:$stateParams.pid}).then(function (res) {
      console.log(res)
      $scope.return=res.data.returns;
      if(res.data.return.status=='Draft'){
        $scope.disable=false;
      }
      else{
        $scope.disable=true;
    ...
    $scope.delete=function(id){
      verifyDelete(id).then(function() {
        $http.post('/return/deletebill',{id:id}).then(function (res) {
          $state.go('returnlist')
