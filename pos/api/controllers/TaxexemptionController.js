...
module.exports = {
	addtaxexe:(req,res)=>{
		Taxexemption.create({reason:req.param('reason'),paragraph:req.param('paragraph'),type:req.param('type')}).exec((err,result)=>{
			if(err) return res.negotiate(err);
			Taxexemption.find({sort:'createdAt DESC'}).exec((err,data)=>{	
	...
	gettaxexees:(req,res)=>{
		...
		Taxexemption.find({sort:'createdAt DESC',limit:10}).exec((err,result)=>{	
			if(err) return res.negotiate(err);
			Taxexemption.count({yourevent:req.session.userId})
	      .exec((err,count)=>{
	...
	getmoretaxexe:(req,res)=>{
	    Taxexemption.find({sort:'createdAt DESC',skip:req.param('skip'),limit:10})
	    .exec((err,finded)=>{
	       Taxexemption.count({yourevent:req.session.userId})
	      .exec((err,count)=>{
	      if(err){return res.negotiate(err)}
	...
	updatetaxexe:(req,res)=>{
	 ...
	 deletetaxexe:(req,res)=>{
	        Taxexemption.destroy({id: req.param('id')}).exec((err, deleted) => {
	          ...
	          Taxexemption.find({sort:'createdAt DESC'})
	            .exec((err, result) => {
	              if (err) {
	                return res.negotiate(err)
	              }
